...
function PresentacionPage() {
  const idPAGINA = 5; //IDENTIFICADOR DE PAGINA UNICO
  useEffect(() => {
  ...
  //Enviar Datos Nueva Presentacion
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(`${config.apiUrl}/agregarPresentacion`, data);     
      if (response.status === 200) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error al agregar presentación',
          showConfirmButton: false,
          timer: 2500
        }).then(() => {
          window.location.reload();
  ...
  const handleEliminar = async () => {
    ...
    try {
      const response = await axios.post(`${config.apiUrl}/cambiarEstadoPresentacion`, { FidPresentacion: idSeleccionado.idPresentacion });     
      if (response.status === 200) {
        Swal.fire({
          icon: 'success',
          title: 'Presentación deshabilitada',
          showConfirmButton: false,
          timer: 1500
        }).then(() => {
          window.location.reload();
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error al deshabilitar presentación',
          showConfirmButton: false,
          timer: 1500
        });
      }
  ...
  const handleHabilitar = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error al habilitar presentación',
          showConfirmButton: false,
          timer: 1500
        });
      }
  ...
  useEffect(() => {
    const getPost = async () => {
      const { data: res } = await axios.get(
        `${config.apiUrl}/consultarPresentacion`
      );
      setPosts(res);
    };
    getPost();
  ...
  return (
    <div className="fondo">
      ...
      <div className="container p-4">
        <h1 className="fondo">Presentación</h1>
          ...
          <table id="tablaDatos" className="table ">
            ...
            <tbody className="table-group-divider">
              {paginatePosts.map((post) => (
                  <tr key={post.idPresentacion}>
                    <td align="center"> {post.idPresentacion}</td>
                    <td> {post.nombre}</td>
                    <td></td>
                    <td></td>
                    <td></td>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <label htmlFor="nombreAgregar" className="font-weight-bold">Ingrese presentación:</label>
                <input className="form-control" placeholder="Presentación" {...register("FNombre", { required: true })} />
                {errors.FNombre && <span className="text-danger">Es necesario agregar la presentación.</span>}
              </div>
              <div className="mb-4"> 
              <br></br> 
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded">
            ...
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ¿Está seguro que desea inhabilitar la siguiente presentación? <br></br> <div style={{ textAlign: 'center', fontSize: '1.2em' }}><b>{idSeleccionado?.nombre}</b></div>
              <br></br>
      ...
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded">
            ...
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ¿Está seguro que desea habilitar la siguiente presentación? <br></br> <div style={{ textAlign: 'center', fontSize: '1.2em' }}><b>{idSeleccionado?.nombre}</b></div>
              <br></br>
