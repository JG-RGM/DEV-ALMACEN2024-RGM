...
function aprobacionRequisicionesPage() {
  const idPAGINA = 9; //IDENTIFICADOR DE PAGINA UNICO
  useEffect(() => {
  ...
  //Aprobar Requisicion
  const aprobarRequisicion = (idRequisicion) => {
    const userData = localStorage.getItem('usuarioDatos');
    const userDataObj = JSON.parse(userData);

    // Hacer la solicitud POST con Axios
    axios.post(`${config.apiUrl}/aprobarRequisicion`, {
      idRequisicion: idRequisicion,
      autorizador: userDataObj.nombre+' '+userDataObj.apellido,
    })
    .then(response => {
  ...
  const rechazarRequisicion = (idRequisicion) => {
    // Hacer la solicitud POST con Axios
    axios.post(`${config.apiUrl}/rechazarRequisicion`, {
      idRequisicion: idRequisicion
    })
    .then(response => {
      if (response.status === 200) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error al rechazar requisicion',
          showConfirmButton: false,
          timer: 2500
        });
      }
  ...
  return (
    <div className="fondo">
      ...
      <div className="container-fluid p-4 " style={{ width: "90%"}}>
        <h1 className="fondo">Aprobación de Requisiciones</h1>
            ...
            <tbody className="table-group-divider">
              {paginatePosts.map((post) => (
                <tr key={post.idRequisicion}>
                  ...
                  <td align="center">
                  {post.estado === 1 ? (
                    <span>Pendiente <i className="bi bi-clock-history" style={{ color: 'blue' }}></i> </span>
                  ) : post.estado === 2 ? (
                    <span>
          ...
          <div className="modal-content rounded">
            ...
            <div className="modal-body">
              ...
              {resultadoDetalleRequisicions ? (
                  <div className="table-container" style={{ maxWidth: '1800px', maxHeight: '800px', overflowY: 'auto' }}>
              ...
              {requisicionSeleccionado.estado === 1 && (
                <div className="d-grid gap-2 d-md-flex justify-content-md-center">
                  <button type="button" className="btn btn-danger" onClick={() => rechazarRequisicion(requisicionSeleccionado.idRequisicion)}>
                    Rechazar Requisicion
                  </button>
                  <button type="button" className="btn btn-success" onClick={() => aprobarRequisicion(requisicionSeleccionado.idRequisicion)}>
                    Aprobar Requisicion
                  </button>
                </div>
              )}
