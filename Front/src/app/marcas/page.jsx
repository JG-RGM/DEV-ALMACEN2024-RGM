...
function MarcasPage() {
  const idPAGINA = 2; //IDENTIFICADOR DE PAGINA UNICO
  useEffect(() => {
  ...
  //Enviar Datos Nueva Marca
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(`${config.apiUrl}/agregarMarca`, data);     
  ...
  const handleEliminar = async () => {
    ...
    try {
      const response = await axios.post(`${config.apiUrl}/cambiarEstadoMarca`, { FidMarca: idSeleccionado.idMarca });     
      if (response.status === 200) {
        Swal.fire({
          icon: 'success',
          ...
          <table id="tablaDatos" className="table ">
            ...
            <tbody className="table-group-divider">
              {paginatePosts.map((post) => (
                  <tr key={post.idMarca}>
                    <td align="center"> {post.idMarca}</td>
                    <td> {post.nombre}</td>
                    <td></td>
                    <td></td>
                    <td></td>
          ...
          <div className="modal-content rounded">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <label htmlFor="nombreAgregar" className="font-weight-bold">Nombre</label>
                <input className="form-control" placeholder="Nombre Marca" {...register("FNombre", { required: true })} />
                {errors.FNombre && <span className="text-danger">Es necesario agregar nombre de la marca.</span>}
              </div>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded">
            ...
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ¿Está seguro que desea inhabilitar la siguiente marca? <br></br> <div style={{ textAlign: 'center', fontSize: '1.2em' }}><b>{idSeleccionado?.nombre}</b></div>
              <br></br>
      ...
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded">
            ...
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ¿Está seguro que desea habilitar la siguiente marca? <br></br> <div style={{ textAlign: 'center', fontSize: '1.2em' }}><b>{idSeleccionado?.nombre}</b></div>
              <br></br>
